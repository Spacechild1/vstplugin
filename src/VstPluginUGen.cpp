...
bool cmdOpen(World *world, void* cmdData) {
	...
	if (data->tryOpen()) {
		LOG_DEBUG("loaded " << data->buf);
		data->value = true;
	}
	else {
		LOG_WARNING("VstPlugin: couldn't load " << data->buf);
		data->value = false;
	}
	return true;
}
